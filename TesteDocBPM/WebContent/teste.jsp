...
<head>
	...
	<link rel="stylesheet" type="text/css" href="bpmn.js/assets/diagram-js.css" />
...
<div id="canvas" style="height: 1200px"></div>
<div id="modals"></div>
...
<script>
    ...
    function decorateTask(taskId, taskName, taskDescription, overlays) {
        var element = document.querySelector('[data-element-id='+'\"' + taskId + '\"'+']');
  		if (element != null) {
  	    	var overlayId = 'overlay-' + taskId;
  	    	var html = '<div class="diagram-note" id="'+overlayId+'">';
    		...
    		html += '</div>';
  	    	...
  	    	document.getElementById('modals').innerHTML += 
  	    		'<div class="modal fade" id="myModal-' + taskId + '" role="dialog">'
  	    	...
  	    	element.addEventListener('click', function(e) {
  				$('#myModal-' + taskId).modal();
  	    	}.bind({overlayId: overlayId}));
  	    	element.addEventListener('mouseover', function(e) {
  	    		document.querySelector('[id='+this.overlayId+']').style.display= "block";
  	    	}.bind({overlayId: overlayId}));
  	    	element.addEventListener('mouseout', function(e) {
  	    		document.querySelector('[id='+this.overlayId+']').style.display= "none";
  	    	}.bind({overlayId: overlayId}));
  			
  		} else {
  			console.log('Id ' + taskId + ' not found in the BPM diagram!');
  		}
  	  
    }
    
  	function decorateTasksByTagName(xmlDoc, overlays, tagName) {
		var taskDesc;
//alert(xmlDoc);
//console.log(xmlDoc.innerHTML);
		tasks = xmlDoc.getElementsByTagName(tagName);
console.log(tasks.length);
//console.log(tagNames[j]);
//console.log(xmlDoc.getElementsByTagName(tagNames[j]));
...
console.log(tasks[i]);
			taskDesc = //tasks[i].querySelectorAll('[name=documentation]');
				tasks[i].getElementsByTagName('bpmn:documentation');
console.log(tasks[i] + "-" + taskDesc);
			if (taskDesc.length != 0) {
//alert(taskDesc);
//				taskDescHtml = taskDesc.textContent;
				decorateTask(tasks[i].getAttribute("id"), tasks[i].getAttribute("name"), taskDesc[0].textContent, overlays);
			}
		}  		
  	}
...
//					'task', 
					'userTask'
//					, 
//					'receiveTask', 'manualTask', 'businessRuleTask', 'serviceTask', 
//					'scriptTask', 'callActivity', 'sendTask', 'subProcess', 
...
//					'intermediateThrowEvent', 'intermediateCatchEvent'
				...
				for (j = 0; j < tagNames.length; j++) {
					decorateTasksByTagName(xmlDoc, overlays, "bpmn:" + tagNames[j]);
				}
...
//				var taskId = 'INSTAURAR_PROC_NOMEACAO_DOCENTE';
//				decorateTask(taskId, taskId, overlays);    
      		});
    	}
  	};
...
//	var bpmnXML = "http://localhost:8080/TesteDocBPM/provimento.bpmn";
